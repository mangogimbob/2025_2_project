...
typedef struct {    // 플레이어 구조체
    ...
    int isDefending;    // 현재 방어중인가?
    int defenseCooldown;    // 전 턴에 방어를 사용하였는가?  1이면 다음 턴 사용 불가
} Character;
...
void drawBattleUI(const Character* player, const Character* enemy, int playerTurn) {    //기본적인 전투UI, 플레이어와 적의 현재 상태를 보여준다
    clearConsole();
    textcolor(15);
    printf("-------------------------------------------------------------------------------------\n");
    ...
    if (activeEffectCount > 0) {
        ...
        for (int i = 0; i < activeEffectCount; i++) {
            ...
            if (strcmp(activeEffects[i].id, "crit_buff") == 0) {
                printf("치확증가(%d턴) ", activeEffects[i].remaining);
            }
            else {
...
void startEffect(const char* id, int duration) {// 효과 활성화 
    if (activeEffectCount >= 16) return;
    // 이미 효과가 있는지 확인 (중복 방지)
    for (int i = 0; i < activeEffectCount; i++) {
...
void showInventoryMenu(Character* player) { // new
    while (1) {
        ...
        else if (strcmp(chosen.id, "focl") == 0) { // 네잎클로버
            startEffect("focl", chosen.duration); // (지속시간은 3이지만, 여기선 횟수로)
...
void doAttack(Character* src, Character* dst, Character* player) {
    ...
    if (hasBlessingById("rare_coinAtk") && src == player) {
        ...
        if (coin == 1) { // 앞면
            textcolor(14);
            printf("희귀 축복: 동전 앞면! 피해량 2배!\n");
            textcolor(15);
            dmg = dmg * 2;
        }
        else { // 뒷면
            textcolor(8);
            printf("희귀 축복: 동전 뒷면... 피해량 절반.\n");
            textcolor(15);
            dmg = dmg / 2;
    ...
    printf("%s의 공격! %d 데미지", src->name, dmg);
    if (isCrit) {
        printf("(");
        textcolor(12);
        printf("치명타!");
    ...
    if (src == player && hasBlessingById("gen_getCritRate")) {
        if (!isCrit) {
            ...
            if (getEffectRemaining("crit_buff") == 0) { // 버프가 중첩되지 않도록
                printf("축복 효과(치확증가): 다음 턴까지 치명타 확률 +50%%!\n");
...
void doDefend(Character* player) { //방어 액션 처리
    if (player->defenseCooldown > 0) {
        textcolor(12);
        printf("이번 턴에는 방어를 사용할 수 없습니다. (남은 쿨다운: %d턴)\n", player->defenseCooldown - 1);
        textcolor(15);
        return;
    }
...
void initBlessings(Blessing arr[], int n) {   // 축복 초기화
    ...
    strcpy(arr[11].id, "rare_coinAtk"); strcpy(arr[11].ext, "매번 공격할때마다 동전을 던진다, 앞면이 나오면 피해량이 두배로 증가하지만 뒷면이 나오면 절반으로 감소한다."); strcpy(arr[11].grade, "희귀");
    arr[11].hpPercent = 0; arr[11].atkPercent = 0.0; arr[11].spdAdd = 0; arr[11].critRateAdd = 0; arr[11].critMultiplierAdd = 0;

    // 전설 효과 (8%)
    strcpy(arr[18].id, "leg_crit45"); strcpy(arr[18].ext, "치명타 확률 +45%"); strcpy(arr[18].grade, "전설");
    arr[18].hpPercent = 0; arr[18].atkPercent = 0; arr[18].spdAdd = 0; arr[18].critRateAdd = 0.45; arr[18].critMultiplierAdd = 0;

    // 익시드 효과 (2%)
    strcpy(arr[23].id, "exc_critrt100"); strcpy(arr[23].ext, "치명타 확률 +100%"); strcpy(arr[23].grade, "???");
    arr[23].hpPercent = 0; arr[23].atkPercent = 0; arr[23].spdAdd = 0; arr[23].critRateAdd = 1.0; arr[23].critMultiplierAdd = 0;
}
...
int main(void) {
    ...
    Blessing allBless[28];
    initBlessings(allBless, 28);
    Item allItems[8];
    initItems(allItems, 8);

    playerInventory.count = 0; // 인벤토리 초기화
    addItemToInventory(&allItems[0]); // bandage 붕대
    // 테스트용 아이템 추가 (필요시 주석 해제)
    // addItemToInventory(&allItems[1]); // energy
    ...
    Character enemies[] = {
    {"굶주린 개", 40, 40, 10, 4, 6, 0.05, 1.5, 0, 0},
    {"커다란 고양이",     60, 60, 15, 6, 5, 0.10, 1.5, 0, 0},
    {"해골 병사",   30, 30, 8,  2, 4, 0.02, 1.3, 0, 0},
    {"돌연변이 주민",    45, 45, 12, 5, 8, 0.07, 1.4, 0, 0}
    };
    ...
    printf("\n축복을 선택하세요.\n");
    for (int i = 0; i < 3; i++) {
        printf("[%d] %s (%s)\n", i + 1, candidates[i].ext, candidates[i].grade);
    }
    int sel = 1;
    char buf[32];
    ...
    while (battleCount < 5) {
        ...
        int playerTurnFirst = (player.spd >= enemy.spd) ? 1 : 0;          // 속도 비교로 선공 결정

        applyPassiveItemsToPlayer_fixed(&player);
        // pauseLine(); 


        while (player.hp > 0 && enemy.hp > 0) {   //배틀 루프 함수, 한 전투 내에서 반복

            if (hasBlessingById("gen_getHP20")) {
                int heal = 10;
                player.hp += heal;
                if (player.hp > player.maxHp) player.hp = player.maxHp;
                textcolor(10);
                printf("축복(턴시작회복) 효과로 HP %d 회복!\n", heal);
                textcolor(15);
                Sleep(700);
            }

            if (getEffectRemaining("energy") > 0) {
            ...
            drawBattleUI(&player, &enemy, playerTurnFirst);     //전투 UI 그리기

            if (playerTurnFirst) {
                player.isDefending = 0;     //플레이어 행동 전에 방어 해제하기
                ...
                while (!valid) {
                    ...
                    case 2:
                        doDefend(&player);
                        if (player.isDefending) { // 방어 쿨다운이 아닐 때만 valid
                            valid = 1;
                        }
                        break;
                    case 3:
                        showInventoryMenu(&player);
                        if (getEffectRemaining("chip_win") > 0) { // 칩 승리
                            enemy.hp = 0; // 적 HP 0으로
                        }
                        drawBattleUI(&player, &enemy, 1);
                        valid = 1;
                        break;
                    default:
                        printf("잘못된 입력입니다.\n");
                ...
                enemy.isDefending = 0;          //쫄몹은 방어X
                doAttack(&enemy, &player, &player);  //적 공격

                if (hasItemInInventory("ddakpend") && player.hp > 0) {
                    int heal = (int)(player.maxHp * 0.15 + 0.0001);
                    player.hp += heal;
                    if (player.hp > player.maxHp) player.hp = player.maxHp;
                    textcolor(10);
                    printf("딱정벌레 펜던트: 피격! HP %d 회복!\n", heal);
                    textcolor(15);
                }
            ...
            tickActiveEffects(&player); // 지속 효과 턴 감소

            if (player.defenseCooldown > 0) {
                player.defenseCooldown--;     // 방어 쿨 1 감소
            }
            if (player.hp < 0) player.hp = 0;       // hp 0으로 조정
        }

        revertPassiveItemsFromPlayer_fixed(&player); // 전투 종료 후 패시브 정상화

        activeEffectCount = 0;  // 지속 효과 초기화
        adrenaline_spd_change = 0; // 아드레날린 스탯 초기화
        forcedHeadsRemaining = 0; // 클로버 횟수 초기화


        if (player.hp <= 0) {
            drawBattleUI(&player, &enemy, 1);
            printf("\n%s(이)가 쓰러졌습니다. 진행한 전투 수: %d\n", player.name, battleCount);
            printf("\n총 가한 데미지");
            if (all_dmg >= 200) {
                textcolor(13);
            }
            ...
            // 전투 승리 후 축복 3개 제시 및 선택
            for (int i = 0; i < 3; i++) {
                int idx = rand() % 28;
                candidates[i] = allBless[idx];
            }
            clearConsole();
            printf("전투 보상: 축복을 선택하세요.\n");
            for (int i = 0; i < 3; i++) {
                printf("[%d] %s (%s)\n", i + 1, candidates[i].ext, candidates[i].grade);
            }
            printf("[0] 받지 않음\n");
